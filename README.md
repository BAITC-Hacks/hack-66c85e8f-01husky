...
Для следующей версии README согласован [сценарий GIF и видео](docs/demo/media-storyboard.md) с расширенным диалогом ru/kk. Запись запланирована после готовности интерфейса и реального пайплайна; ниже пока доступен образец из fake-фикстуры.
...
| Диаризация и привязка поручения к человеку | Акустические группы speaker1, speaker2; имена по передаче слова и вручную | Локальная диаризация и контекстная привязка подключены; требуют проверки человеком |
| Поручения с ответственным и сроком | Цитата → суть → человек → календарная дата | Локальная Qwen3:8b, проверка дословных цитат, нормализация сроков, черновики |
| Саммари | Обсуждённые проблемы и решения | Генерация локальной Qwen3; backend сохраняет и экспортирует |
...
| Слой | Технологии |
|---|---|
| Пайплайн | Python 3.12, faster-whisper, sherpa-onnx (pyannote segmentation + TitaNet), Ollama (Qwen3:8b), pydantic v2 |
...
Требования: Docker с Compose v2; локальная Qwen3:8b проверяется на Mac с 16 ГБ RAM.
Настройка без Docker, приватность и ограничения: [локальная LLM](docs/local-llm.md).
...
| Диаризация | Локальные pyannote segmentation + TitaNet через sherpa-onnx; speaker1/speaker2, разделение по таймкодам слов |
| Локальная LLM | Qwen3:8b через Ollama: поручения, саммари, имена по явной передаче слова; цитаты проверяются кодом |
| Voiceprint-идентификация | Эталонное узнавание человека ещё не реализовано; контекстные имена имеют источник llm, не voiceprint |
