...
![Whisper](https://img.shields.io/badge/STT-faster--whisper-6557FF)
![On-premise](https://img.shields.io/badge/данные-не%20покидают%20контур-1f9d6b)
![Languages](https://img.shields.io/badge/язык-RU%20·%20KK%20·%20шала--казахский-6557FF)

[▶ Демо-видео](demo/Kenes-AI-Demo-v2.mp4) · [Попробовать за минуту](#попробовать-за-минуту) · [Демо-протокол PDF](docs/demo/protocol.pdf) · [Скриншоты](#интерфейс) · [Архитектура](#архитектура) · [Статус](#что-готово)

<a href="demo/Kenes-AI-Demo-v2.mp4"><img src="docs/screenshots/demo-video-cover.png" alt="Смотреть демо-видео Kenes AI, 44 секунды" width="900"></a>

<sub>Нажмите, чтобы посмотреть 44-секундное демо: от ссылки на встречу до поручения под контролем</sub>

</div>

## Зачем

После совещания секретарь час-два переслушивает запись, выписывает поручения и рассылает их вручную. Поручения теряются, сроки «до пятницы» трактуются по-разному. В Казахстане задача сложнее: участники переходят с русского на казахский **внутри одной фразы**, а облачные сервисы протоколирования часто нельзя использовать из-за требований к данным.

**Kenes AI** («кеңес» — совет, совещание) превращает запись встречи в черновик протокола:

1. **Распознаёт речь** на русском, казахском и смешанном языке локальной моделью Whisper.
2. **Разделяет говорящих** по голосу и связывает реплики с участниками.
3. **Выделяет поручения**: что сделать, кто отвечает, к какой календарной дате. У каждого поручения есть **цитата-основание** из реплики.
4. **Даёт секретарю проверить** черновик: поправить ответственного или срок и утвердить протокол.
5. **Выгружает протокол** в DOCX/PDF, отправляет его в СЭД, уведомляет исполнителей и напоминает о сроках.

Модели работают локально. Аудио и текст не передаются во внешние облачные API.

## Как это работает: 44 секунды

[Видео](demo/Kenes-AI-Demo-v2.mp4) показывает путь одной встречи через всю систему:

**Ссылка → Бот → Аудио → Очередь → ИИ → Черновик → Проверка → Утверждение → Исполнение**

<table>
<tr>
<td width="33%"><a href="demo/Kenes-AI-Demo-v2.mp4"><img src="docs/screenshots/demo-frame-pipeline.png" alt="Backend проверяет загрузку и ставит аудио в очередь"></a><br><sub><b>Приём.</b> Бот передаёт запись, backend проверяет её и ставит в очередь обработки.</sub></td>
<td width="33%"><a href="demo/Kenes-AI-Demo-v2.mp4"><img src="docs/screenshots/demo-frame-store.png" alt="Транскрипт, поручения и основания сохраняются в одной записи"></a><br><sub><b>Одна запись.</b> Транскрипт, поручения и цитаты-основания хранятся вместе.</sub></td>
<td width="33%"><a href="demo/Kenes-AI-Demo-v2.mp4"><img src="docs/screenshots/demo-frame-followup.png" alt="Назначение, напоминание о сроке и просрочка"></a><br><sub><b>Контроль.</b> Назначение, напоминание о сроке, уведомление о просрочке.</sub></td>
</tr>
</table>
...
<img src="docs/screenshots/meeting-detail.png" alt="Карточка совещания: хронология выступлений, транскрипт с метками языка и поручения с цитатой-основанием" width="100%">
...
<table>
<tr>
<td width="50%"><img src="docs/screenshots/meetings.png" alt="Реестр совещаний со статусами"><br><sub><b>Реестр протоколов.</b> Источник записи, длительность, язык, статус обработки.</sub></td>
<td width="50%"><img src="docs/screenshots/meetings-new.png" alt="Создание совещания"><br><sub><b>Новое совещание.</b> Загрузка файла, запись с микрофона или бот во встречу.</sub></td>
</tr>
<tr>
<td><img src="docs/screenshots/tasks.png" alt="Дашборд поручений"><br><sub><b>Контроль исполнения.</b> Счётчики, фильтры по ответственному, направлению, срочности.</sub></td>
<td><img src="docs/screenshots/tasks-board.png" alt="Канбан поручений"><br><sub><b>Доска поручений.</b> От черновика до выполнения; просрочка подсвечена.</sub></td>
</tr>
<tr>
<td><img src="docs/screenshots/meeting-detail-dark.png" alt="Карточка совещания в тёмной теме"><br><sub><b>Тёмная тема.</b> Хронология выступлений и доля русского, казахского и смешанной речи.</sub></td>
<td><img src="docs/screenshots/login.png" alt="Экран входа"><br><sub><b>Вход.</b> RU / ҚАЗ переключаются в один клик.</sub></td>
</tr>
</table>
...
Мы разделяем то, что уже работает, и то, что остаётся сделать.

✅ работает и покрыто тестами · 🟡 работает частично или проверено не полностью · 🔜 в разработке
...
| Проверка секретарём и утверждение протокола | ✅ | Правка спикеров, ответственных, сроков; статусы поручений |
| Дашборд, канбан, напоминания о сроках | ✅ | Celery beat раз в час отмечает `due_soon` и `overdue` |
| СЭД | ✅ | MockSED выдаёт регистрационный номер; интерфейс `SEDClient` готов для реальной СЭД |
| Распознавание речи RU / KK / смешанной | ✅ | Локальный faster-whisper `large-v3-turbo`, CPU int8, без сети |
| Маскирование телефонов и ИИН | ✅ | Применяется до сохранения транскрипта |
| Диаризация (кто говорит) | ✅ | Локально: pyannote segmentation 3.0 + NeMo TitaNet (sherpa-onnx) |
...
- Поручения и саммари из настоящей записи ещё не извлекаются. Контракт, хранение, редактирование и экспорт готовы и работают на fake-пайплайне.
...
- Извлечение поручений и саммари локальной LLM с проверкой цитат по исходной реплике.
