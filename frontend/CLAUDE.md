...
pnpm gen:api    # openapi-typescript from ${BACKEND_URL}/openapi.json → src/lib/api/schema.gen.ts (backend must be running)
```

Mock data persists in localStorage. Open any page with `?reset-mocks` to start over. Demo login: `admin@kenes.ai`, any password.
Real backend (`pnpm dev`, see `../docs/local-development.md`): seed login `admin@example.com` / `admin123`. Open the app on `localhost`, not `127.0.0.1`: the WebSocket goes straight to the backend and needs the same-host cookie.
...
src/lib/api/schema.gen.ts  generated by `pnpm gen:api`; never edit by hand
src/lib/api/types.ts       app-facing aliases over schema.gen.ts (+ narrowed unions, `hasAudio()`)
...
- **New endpoint:** run `pnpm gen:api`, alias the schema in `types.ts`, add a hook to `queries/`, and a handler + fixture to `mocks/`. `pnpm mock` must keep working. Mock handlers return the same shapes as the backend (flat `MeetingDetail`, `progress_pct` 0–100), typed with the generated types.
