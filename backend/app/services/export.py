...
def build_docx(meeting_detail: Any, lang: str) -> Path:
    ...
    participant_names = [
        _display(_value(person, "name")) for person in participants if _value(person, "name")
    ]
    ...
    table.autofit = False
    widths = (0.7, 3.0, 5.5, 2.2, 2.6, 3.1)
    ...
    if segments:
        ...
        for mapping in _as_list(_value(meeting_detail, "speaker_map", default=[])):
            speaker = _value(mapping, "speaker")
            participant_id = _value(mapping, "participant_id")
            for person in participants:
                if participant_id is not None and _value(person, "id") == participant_id:
                    speaker_names[str(speaker)] = _display(_value(person, "name"))
                    break
