...
## Проверка записи и реальной транскрипции

23 сентября 2026 проверен Linux bot-runtime в Colima. Стандартный seccomp Docker мешал запуску sandbox Chromium. Подключён неизменённый [профиль Playwright v1.63.0](../bots/runtime/README.md) только для bot-worker, сохранены non-root и sandbox. Проверка через Compose записала тон 439.9 Hz, RMS 0.1415, WAV 16 kHz mono.

Дополнительно headed Chromium воспроизвёл синтетическую контрольную речь. Production lifecycle бота записал её через PulseAudio и ffmpeg. Полученный WAV длиной 7.363 секунды загружен через настоящий HTTP callback в отдельную тестовую схему backend. Пайплайн работал с `PIPELINE_FAKE=0`: faster-whisper large-v3-turbo и sherpa-onnx diarization, без сетевых вызовов моделей.

| Проверка | Результат |
|---|---|
| Контрольный текст | «Проверка записи совещания. Подготовьте отчёт к пятнице. Следующая встреча состоится в десять утра.» |
| Распознавание | Все фразы сохранены; «отчёт» записан как «отчет», «десять» как «10» |
| Диаризация | Один спикер, три сегмента; личность не привязана |
| Callback и БД | HTTP 200, сохранён `draft`, три сегмента в БД |
| Повторный callback | HTTP 409 |
| Очистка | Тестовый API остановлен, схема и серверная копия аудио удалены |

Backend-проверка использовала `CELERY_EAGER=1`, поэтому доставку через Redis она не подтверждает. Запись не проходила через внешний Meet/Teams/Zoom. После допуска в Meet остаётся записать живую речь, сопоставить её с транскриптом и проверить очередь обработки. Смешанная ru/kk речь и смена нескольких спикеров этой контрольной фразой не проверены.
