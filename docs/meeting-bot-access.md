...
Повторная проверка в штатном Linux bot-runtime с sandbox, Xvfb и светлой камерой дала тот же результат: `ResolveMeetingSpace` 200, автоматическая отправка имени `Kenes AI`, затем `CreateMeetingDevice` 403. В protobuf-ответе сервера содержится `The caller does not have permission`. Повтор после получения ссылки от организатора тоже отклонён. Это подтверждает серверный отказ и воспроизведение на двух ОС, но не устанавливает, какая политика доступа его вызвала. Следующий контроль требует временного режима Open тестовой встречи либо отдельной авторизованной учётной записи; изменение режима запрошено у организатора.

Организатор сообщил о переключении в Open. Первая попытка затем завершилась UI timeout; повтор в Linux снова дал тот же 403, `in_call=false`. Отдельно запущенный обычный Chrome с пустым профилем также показал форму имени и отклонил заявку. Эффективность новых настроек для уже открытого экземпляра встречи не подтверждена: исходный экран предупреждал о применении к будущим запускам. Запрошена новая встреча с другим кодом либо отдельный аккаунт Kenes AI. Записи живого Meet нет; эти отказы не считаются успешной приёмкой.
...
После пересборки текущего кода повторный прогон дал тот же текст, одного спикера и один сегмент вместо трёх; длина WAV 7.336 секунды. Разбиение короткой записи на сегменты меняется, содержание трёх фраз сохранено. Callback 200, сохранение `draft` и защита от повторной загрузки 409 подтвердились повторно.
