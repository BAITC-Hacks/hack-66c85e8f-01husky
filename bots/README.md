...
Бот подключается к онлайн-встрече гостем с именем **Kenes AI**, записывает звук и передаёт запись в backend для протоколирования. Секретарю достаточно вставить ссылку на встречу. Организатор допускает бота из комнаты ожидания, как обычного участника. Камера бота показывает логотип Kenes AI, микрофон всегда без звука.

Гостевые адаптеры Google Meet, Zoom и Microsoft Teams написаны на Playwright. Ответственный: Никита. [Главный README](../README.md) · [Исследование способов подключения](../docs/meeting-bot-access.md)

| Платформа | Статус |
|---|---|
| Microsoft Teams | ✅ Живой гостевой вход подтверждён: бот виден участникам как «Kenes AI (Guest)» |
| Google Meet | 🟡 Адаптер готов, имя вводится автоматически; сервер тестовой встречи отклонил гостевую заявку (403), нужна другая встреча или отдельный аккаунт |
| Zoom | 🟡 Адаптер готов; тестовая встреча запрещает автоматических ботов. Официальный путь — Zoom RTMS |
| Аудиотракт | ✅ Chromium → PulseAudio → ffmpeg → callback → Whisper и диаризация проверены в Linux-контейнере на контрольной речи |

Запись живого звонка с загрузкой в backend ещё не подтверждена. Подробный журнал проверок: [отчёт о доступе](../docs/meeting-bot-access.md).
